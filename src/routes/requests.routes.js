...
r.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (food.donor?.email && food.donor.email === user.email) {
...
r.get(
  ...
  asyncHandler(async (req, res) => {
    const email = req.user.email
    const requests = await Request.find({ donorEmail: email })
      .sort({ createdAt: -1 })
      .populate('food')
    res.json(requests)
  })
...
r.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!raw) return res.status(400).json({ message: 'Missing status' })

    const normalized = String(raw).trim().toLowerCase()
    const map = { pending: 'Pending', accepted: 'Accepted', rejected: 'Rejected' }
    const status = map[normalized] || raw
