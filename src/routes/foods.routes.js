...
r.get('/featured', asyncHandler(async (req, res) => {
  const foods = await Food.find({ status: 'Available' });
  const top = foods.map(f=>({f,score:parseInt((f.quantity.match(/\d+/)||['0'])[0],10)}))
...
r.post('/', requireAuth, asyncHandler(async (req, res) => {
  ...
  const food = await Food.create({
    name, imageUrl, quantity, pickupLocation, expireDate, notes,
    donor: { name: req.user.name || req.user.email, email: req.user.email, photoURL: req.user.picture, uid: req.user.uid },
    status: 'Available'
  });
